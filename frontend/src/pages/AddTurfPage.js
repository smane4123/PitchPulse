...
// --- LEAFLET HELPER COMPONENTS (Unchanged) ---
...
// --- BACKGROUND WRAPPER COMPONENT (Unchanged) ---
    ...
    return (
        <>
            <style>{`
                ...
                .form-input, .form-textarea, .form-select { width: 100%; padding: 0.8rem 1rem; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem; transition: border-color 0.2s, box-shadow 0.2s; background-color: white; }
...
export default function AddTurfPage() {
    ...
    // --- UPDATED: Added openingTime and closingTime to the initial state ---
    const [turfData, setTurfData] = useState({
        ...
        pricePerHour: "", 
        location: null,
        openingTime: "06:00", // Default opening time
        closingTime: "23:00", // Default closing time
    });
        ...
        if (isEditMode) {
            const fetchTurfData = async () => {
                try {
                    ...
                    // Set all data, including the new time fields if they exist
                    setTurfData({ 
                        ...data, 
                        location: data.location.coordinates,
                        // Ensure defaults if old data doesn't have these fields
    ...
    // Geocoding and other handlers remain unchanged
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('turfData', JSON.stringify({
            ...turfData, // This will now include openingTime and closingTime
            pricePerHour: parseFloat(turfData.pricePerHour),
            location: { type: 'Point', coordinates: turfData.location }
        }));
        if (images.length > 0) images.forEach(image => formData.append('images', image));
            ...
            <div className="add-turf-card">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="form-grid">
                        <div className="form-group"><label className="form-label" htmlFor="name">Turf Name</label><input id="name" type="text" name="name" value={turfData.name} onChange={handleChange} required className="form-input" /></div>
                        <div className="form-group"><label className="form-label" htmlFor="sport">Primary Sport</label><select id="sport" name="sport" value={turfData.sport} onChange={handleChange} required className="form-select"><option>Football</option><option>Cricket</option><option>Badminton</option><option>Basketball</option><option>Tennis</option><option>Mixed</option></select></div>
                        <div className="form-group"><label className="form-label" htmlFor="contact">Contact Number</label><input id="contact" type="tel" name="contact" value={turfData.contact} onChange={handleChange} required className="form-input" /></div>
                        <div className="form-group"><label className="form-label" htmlFor="pricePerHour">Price per Hour (₹)</label><input id="pricePerHour" type="number" name="pricePerHour" value={turfData.pricePerHour} onChange={handleChange} required className="form-input" /></div>
                        
                        {/* --- ADDED: Opening and Closing Time Fields --- */}
                        ...
                        <div className="form-group full-width"><label className="form-label" htmlFor="address">Address</label><div className="address-group"><textarea id="address" name="address" value={turfData.address} onChange={handleChange} required className="form-textarea" rows={2}/><button type="button" onClick={forwardGeocode} className="btn btn-secondary">Find</button></div></div>
                        <div className="form-group full-width"><label className="form-label">Amenities</label><div className="amenities-grid">{["Floodlights", "Parking", "Washrooms", "Seating", "Drinking Water"].map(amenity => ( <label key={amenity} className="amenity-label"><input type="checkbox" value={amenity} checked={turfData.amenities.includes(amenity)} onChange={handleAmenityChange} />{amenity}</label> ))}</div></div>
                        <div className="form-group full-width"><label className="form-label">Turf Images (Max 5)</label><label htmlFor="image-upload" className="image-upload-box"><FaUpload style={{ marginRight: '8px' }} />{isEditMode ? 'Upload New to Replace' : 'Click or Drag to Upload'}</label><input id="image-upload" type="file" accept="image/*" multiple onChange={handleImageChange} style={{ display: 'none' }} /><div className="image-preview-grid">{imagePreviews.map((src, index) => ( <img key={index} src={src} alt={`Preview ${index + 1}`} className="image-preview-item" /> ))}</div></div>
                        <div className="form-group full-width"><label className="form-label">Set Location on Map</label><MapContainer center={[mapCenter.latitude, mapCenter.longitude]} zoom={mapCenter.zoom} scrollWheelZoom={true}><TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" /><ChangeMapView center={mapCenter} /><LocationMarker setTurfData={setTurfData} reverseGeocode={reverseGeocode} initialLocation={turfData.location} /></MapContainer></div>
                    </div>
